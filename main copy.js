...
function setupAckHandler(client) {
  client.on('message_ack', async (msg) => {
    ...
    try {
      await updateAckStatus(msg.id._serialized, status);
...
function startWhatsAppService(customChromePath = null) {
  ...
  client.on('qr', async (qr) => {
    ...
    if (tray)
      tray.setToolTip('Monitor WhatsApp - Aguardando leitura do QR Code');
  ...
  client.on('ready', () => {
    ...
    setInterval(startQueueProcessing, 30000);
  });
  ...
  client.on('disconnected', () => {
    ...
    setTimeout(() => startWhatsAppService(customChromePath), 10000); // Passa o caminho novamente na reconexão
  });
...
app.whenReady().then(() => {
  ...
  createTray();
  startWhatsAppService(customChromePath);
